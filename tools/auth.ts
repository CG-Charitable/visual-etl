...
// (carried through OAuth's `state` param) — never follow an absolute or
...
// Hydrate `auth` from disk at startup. Without this, the OAuth callback
// paths below (Google/Microsoft/CAS) — which only lazily create an entry
// for the logging-in user and never call loadAuth() themselves — would
// start from an empty in-memory `auth` on a freshly-started server, and
// saveAuth() would then overwrite auth.json with just that one user,
// silently deleting every other locally-stored account.
await loadAuth();
...
export function setupPublicRoutes(app: Hono): void {
  ...
  app.get("/auth/google", (c) => {
    const dest = safeRedirectPath(c.req.query("redirect"));
  ...
  app.get("/auth/callback/google", async (c) => {
    ...
    try {
      ...
      await store.set(token, session);
      setAuthCookie(c, token);
      return c.redirect(safeRedirectPath(c.req.query("state")));
    } catch (error) {
  ...
  app.get("/auth/microsoft", (c) => {
    ...
    const params = new URLSearchParams({
      ...
      state: safeRedirectPath(c.req.query("redirect")),
    });
